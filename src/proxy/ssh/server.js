...
const { getSSHConfig, getProxyUrl } = require('../../config');
...
class SSHServer {
  constructor() {
    this.server = new ssh2.Server(
      {
        hostKeys: [require('fs').readFileSync(getSSHConfig().hostKey.privateKeyPath)],
    ...
    session.on('exec', async (accept, reject, info) => {
      ...
      if (command.startsWith('git-')) {
        ...
        try {
          ...
          const remoteUrl = new URL(getProxyUrl());
          ...
          if (clientKey) {
            ...
            if (typeof clientKey === 'object' && clientKey.algo && clientKey.data) {
              ...
              connectionOptions.privateKey = require('fs').readFileSync(
                getSSHConfig().hostKey.privateKeyPath,
              );
          ...
          remoteGitSsh.on('error', (err) => {
            ...
            if (
              ...
              connectionOptions.privateKey !==
                require('fs').readFileSync(getSSHConfig().hostKey.privateKeyPath)
            ) {
              ...
              const proxyConnectionOptions = {
                ...connectionOptions,
                privateKey: require('fs').readFileSync(getSSHConfig().hostKey.privateKeyPath),
