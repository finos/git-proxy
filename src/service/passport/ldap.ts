...
const createClient = (ldapConfig: LDAPConfig): Client => {
  return new Client({
    ...
    strictDN: true,
    ...(typeof ldapConfig.timeout === 'number' ? { timeout: ldapConfig.timeout } : {}),
    ...(typeof ldapConfig.connectTimeout === 'number'
      ? { connectTimeout: ldapConfig.connectTimeout }
      : {}),
  });
};

const getSearchOptions = (ldapConfig: LDAPConfig, filter: string) => ({
  scope: 'sub' as const,
  filter,
  ...(typeof ldapConfig.searchTimeLimit === 'number'
    ? { timeLimit: ldapConfig.searchTimeLimit }
    : {}),
  ...(typeof ldapConfig.searchSizeLimit === 'number'
    ? { sizeLimit: ldapConfig.searchSizeLimit }
    : {}),
});
...
export const searchUser = async (
  ...
  const { searchEntries } = await client.search(
    ...
    getSearchOptions(ldapConfig, filter),
  );
...
export const isUserInGroup = async (
  ...
  const { searchEntries } = await client.search(
    searchBase,
    getSearchOptions(ldapConfig, `(&(objectClass=*)${groupFilter})`),
  );

  return searchEntries.some(
    (entry: Record<string, unknown>) =>
      typeof entry.dn === 'string' && entry.dn.toLowerCase() === groupDN.toLowerCase(),
  );
};
...
export const authenticateUser = async (
  ...
  try {
    ...
    // Step 4: Verify user's password before doing group authorization work
    const passwordValid = await verifyPassword(ldapConfig, userDN, password);
    if (!passwordValid) {
      return null;
    }

    // Step 5: Check required user group membership
    const isMember = await isUserInGroup(
      client,
      ldapConfig,
      userDN,
      ldapConfig.userGroupDN,
      username,
    );
    if (!isMember) {
    ...
    if (!email) {
      throw new Error(`LDAP user "${username}" is missing required email attribute "${emailAttr}"`);
    }
...
export const configure = async (passport: PassportStatic): Promise<PassportStatic> => {
  ...
  passport.use(
    type,
    new CustomStrategy(async (req: Request, done: (error: unknown, user?: unknown) => void) => {
