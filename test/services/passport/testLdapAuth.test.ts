...
describe('LDAP auth method', () => {
  ...
  it('should authenticate a valid user and mark them as admin', async () => {
    ...
    expect(serviceClientMock.search).toHaveBeenNthCalledWith(
      1,
  ...
  it('should fail if user password is incorrect (user bind fails)', async () => {
    serviceClientMock.search
      ...
      // Membership checks should not run for a bad password.
      .mockRejectedValue(new Error('Unexpected group lookup'));
    ...
    expect(user).toBe(false);
    expect(dbStub.updateUser).not.toHaveBeenCalled();
    expect(serviceClientMock.search).toHaveBeenCalledOnce();
  });

  it('should surface required user group lookup errors', async () => {
    serviceClientMock.search
      .mockResolvedValueOnce({
        searchEntries: [
          {
  ...
  it('should continue with admin=false if admin group lookup fails', async () => {
    serviceClientMock.search
      .mockResolvedValueOnce({
        searchEntries: [
          {
      ...
      .mockRejectedValueOnce(new Error('Admin group search failed'));
  ...
  it('should fail if the LDAP email attribute is missing', async () => {
    serviceClientMock.search
      .mockResolvedValueOnce({
        searchEntries: [
          {
            dn: 'uid=noemail,ou=people,dc=test,dc=com',
            uid: 'noemail',
            cn: 'No Email',
            title: '',
          },
    ...
    const req = { body: { username: 'noemail', password: 'pass' } };
    const done = vi.fn();
    ...
    expect(done).toHaveBeenCalledOnce();
    const [err, user] = done.mock.calls[0];
    expect(err).toContain('missing required email attribute');
    expect(user).toBeUndefined();
    expect(dbStub.updateUser).not.toHaveBeenCalled();
  });
...
describe('escapeFilterValue', () => {
  ...
  it('should support username placeholders in LDAP group filters', async () => {
    const mod = await import('../../../src/service/passport/ldap.js');
    const client = {
    ...
    const result = await mod.isUserInGroup(
      client as any,
      {
        ...ldapConfig,
    ...
    expect(client.search).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        filter: '(&(objectClass=*)(memberUid=test\\2auser))',
      }),
    );
  });
});
