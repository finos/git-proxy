...
These server settings can also be set in the [configuration file](./reference) using the
`serverPort`, `httpsServerPort`, `uiHost`, `uiPort` and `httpsUiPort` properties. When a value is
provided in both places, the environment variable takes precedence over the config file.
