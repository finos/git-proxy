...
GitProxy uses [Vitest](https://vitest.dev/) as the test runner. User interface tests are written in [Cypress](https://docs.cypress.io), and some fuzz testing is done with [`fast-check`](https://fast-check.dev/).
...
The `RUN_MONGO_TESTS=true` environment variable is required to enable the MongoDB-specific tests locally. In CI, this is set in the workflow step that runs integration tests.
