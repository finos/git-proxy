...
GitProxy applies a series of automated checks (called [Processors](processors.md)) to every push. These checks run in sequence, and if any check fails, the push is blocked. The main checks include:
