...
- Any other git server accessible via HTTPS

This broad compatibility is possible because GitProxy operates at the Git protocol level rather than relying on proprietary SCM platform APIs. As long as the destination supports standard git operations over HTTPS, GitProxy can proxy traffic to it.

:::note
Currently, only HTTPS-based git operations are supported. SSH-based git operations are under active development.
:::
...
2. GitProxy is configured for the forked project (with nominated contributors and reviewers), allowing you to clone, fetch and pull the project through the proxy. The GitProxy URL is added to your local clone as a *remote*.
...
- **Proxy authentication (for git operations):** You authenticate using a **Personal Access Token (PAT)** from the target SCM platform (e.g., GitHub PAT). GitProxy forwards these credentials when cloning repositories to calculate diffs and when pushing approved changes upstream. For GitHub you'll need a personal access token (classic) with `repo` permissions. See [GitHub's documentation](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens#creating-a-personal-access-token-classic) for details.
...
:::danger Do not disable SSL verification
Never set `http.sslVerify=false` — it is insecure and leaves you vulnerable to man-in-the-middle attacks.
:::

If your organisation uses internal Certificate Authorities (CAs) for SSL certificates on internal infrastructure, tools like git may not recognise these CAs by default, causing certificate errors. The correct fix is to configure git to use your operating system's certificate store, which includes the internal CAs.
