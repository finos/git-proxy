...
- The approval is specific to the repository, the branch or tag, and the commit SHA(s) that were reviewed. Pushing the same commits to a different branch or repository, or pointing the tag at a different commit, requires a new review.
- Each push may update a single ref (one branch or one tag), thus multiple ref updates are rejected.
...
- You are pushing the exact same commits that were approved (same SHA) to the same repository and branch
